...
export class VeilClient {
  ...
  async withdraw(
    ...
    const relayerAddress =
      useRelayer && this.relayer ? this.config.relayerPublicKey : recipient;
    if (useRelayer && this.relayer && !relayerAddress) {
      throw new Error("relayerPublicKey is required when using the relayer");
    }
    const effectiveRelayerAddress = relayerAddress || recipient;
    ...
    const tx = new StellarSdk.TransactionBuilder(account, {
      ...
      .addOperation(
        contract.call(
          ...
          StellarSdk.nativeToScVal(effectiveRelayerAddress, {
            type: "address",
          }),
  ...
  async syncTree(): Promise<void> {
    // During local development, we sync from the API endpoint.
    // A production setup would typically use Stellar event subscriptions.
