/**
 * End-to-end demo for a locally configured environment.
 *
 ...
 *
 * Prerequisites:
 *   - Contract IDs and keys provided via environment variables
...
const SUBMITTER_SECRET_KEY = process.env.SUBMITTER_SECRET_KEY || "";
const SKIP_FRESH_DEPLOY = process.env.SKIP_FRESH_DEPLOY === "true";
...
async function main() {
  ...
  if (!fs.existsSync(CIRCUIT_WASM) || !fs.existsSync(CIRCUIT_ZKEY)) {
    ...
    console.error(
      "  ./scripts/compile-circuit.sh && ./scripts/setup-ceremony.sh",
    );
    process.exit(1);
  }
  ...
  if (deployerSecret && !SKIP_FRESH_DEPLOY) {
    ...
    function pointToG1Hex(point: string[]): string {
      return (
        bigintToHex(BigInt(point[0]), 32) + bigintToHex(BigInt(point[1]), 32)
      );
    }
    ...
    if (!poolContractId || !verifierContractId || !TOKEN_CONTRACT_ID) {
      throw new Error(
        "Set POOL_CONTRACT_ID, VERIFIER_CONTRACT_ID, and TOKEN_CONTRACT_ID in the environment, or provide DEPLOYER_SECRET_KEY for fresh deployment.",
      );
    }
  ...
  if (StellarSdk.SorobanRpc.Api.isSimulationError(simulated)) {
    const errResp =
      simulated as StellarSdk.SorobanRpc.Api.SimulateTransactionErrorResponse;
  ...
  try {
    ...
    if (rawData.result?.status === "SUCCESS") {
      ...
      console.error(
        "  Raw result:",
        JSON.stringify(rawData.result, null, 2).slice(0, 500),
      );
      process.exit(1);
    }
  }
  ...
  try {
    ...
    if (!StellarSdk.SorobanRpc.Api.isSimulationError(nextIdxSim)) {
      const successSim =
        nextIdxSim as StellarSdk.SorobanRpc.Api.SimulateTransactionSuccessResponse;
  ...
  const submitterSecret =
    SUBMITTER_SECRET_KEY || RELAYER_SECRET_KEY || deployerSecret || "";
  const submitter = submitterSecret
    ? StellarSdk.Keypair.fromSecret(submitterSecret)
    : recipient;
  const proofRelayerAddress =
    RELAYER_PUBLIC_KEY ||
    (submitterSecret ? submitter.publicKey() : recipient.publicKey());
  const recipientField = await addressToField(recipient.publicKey());
  const relayerField = await addressToField(proofRelayerAddress);
  const fee = 0n;
  ...
  if (useRelayer) {
    ...
    if (relayData.success) {
      console.log(
        `  Withdrawal queued! Estimated delay: ${relayData.estimatedDelay}s`,
      );
    ...
    if (!submitterSecret) {
      console.error("  No submitter secret configured for direct submission");
      console.log(
        "\n  Set SUBMITTER_SECRET_KEY (or RELAYER_SECRET_KEY / DEPLOYER_SECRET_KEY), or start the relayer.",
      );
      process.exit(0);
    }
    ...
    if (StellarSdk.SorobanRpc.Api.isSimulationError(withdrawSim)) {
      const errResp =
        withdrawSim as StellarSdk.SorobanRpc.Api.SimulateTransactionErrorResponse;
    ...
    if (withdrawStatus === "SUCCESS") {
      ...
      console.log(
        `  TX: https://stellar.expert/explorer/testnet/tx/${withdrawSend.hash}`,
      );
    } else {
