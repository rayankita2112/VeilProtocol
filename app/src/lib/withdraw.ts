...
function bigintToU256ScVal(val: bigint): StellarSdk.xdr.ScVal {
  ...
  return StellarSdk.xdr.ScVal.scvU256(
    new StellarSdk.xdr.UInt256Parts({
      hiHi: StellarSdk.xdr.Uint64.fromString(
        BigInt("0x" + hex.slice(0, 16)).toString(),
      ),
      hiLo: StellarSdk.xdr.Uint64.fromString(
        BigInt("0x" + hex.slice(16, 32)).toString(),
      ),
      loHi: StellarSdk.xdr.Uint64.fromString(
        BigInt("0x" + hex.slice(32, 48)).toString(),
      ),
      loLo: StellarSdk.xdr.Uint64.fromString(
        BigInt("0x" + hex.slice(48, 64)).toString(),
      ),
    }),
  );
}
...
export async function checkSubsetStatus(
  commitment: bigint,
): Promise<SubsetStatus> {
...
export async function executeWithdraw(
  ...
  console.log(
    `[withdraw] Note leafIndex=${leafIndex}, commitment=${commitmentHex.slice(0, 16)}...`,
  );
  console.log(
    `[withdraw] Store[${leafIndex}]=${allCommitmentHexes[leafIndex]?.slice(0, 16) ?? "N/A"}...`,
  );
  console.log(
    `[withdraw] Local commitments: ${localCount}, on-chain: ${onChainCount}`,
  );
  ...
  if (allCommitmentHexes[leafIndex] !== commitmentHex) {
    ...
    if (foundAt >= 0) {
      ...
      if (partialMatch >= 0) {
        console.log(
          `[withdraw] Case-insensitive match at position ${partialMatch}. Using it.`,
        );
  ...
  const fullResult = buildFullTreeAndPath(
    allLeaves,
    verifiedLeafIndex,
    poseidon,
  );
  ...
  if (fullResult.root === onChainRoot || knownRoots.has(fullRootHex)) {
    ...
    if (fullResult.root === onChainRoot) {
      ...
      console.log(
        `[withdraw] Full tree root matches historical root: ${fullRootHex.slice(0, 16)}...`,
      );
    }
  } else {
    ...
    for (
      let size = allLeaves.length - 1;
      size >= verifiedLeafIndex + 1;
      size--
    ) {
      const partialLeaves = allLeaves.slice(0, size);
      const partial = buildFullTreeAndPath(
        partialLeaves,
        verifiedLeafIndex,
        poseidon,
      );
  ...
  let subsetProofResult: { proof: unknown; publicSignals: string[] } | null =
    null;
  try {
  ...
  try {
    ...
    const freshAccount = new StellarSdk.Account(
      senderAddress,
      freshData.sequence,
    );
    ...
    try {
      ...
      console.log("[withdraw] Could not log auth:", (e as Error).message);
    }
  } catch (assembleErr) {
    console.log(
      "[withdraw] SDK assemble failed, using manual rebuild:",
      (assembleErr as Error).message,
    );
  ...
  if (!sendResult || sendResult.status === "ERROR") {
    throw new Error(
      `Submission failed: ${sendResult?.errorResultXdr ?? "unknown"}`,
    );
  }
