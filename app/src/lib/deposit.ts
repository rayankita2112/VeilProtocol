    ...
    for (const authEntry of successSim.result.auth) {
      const entry =
        typeof authEntry === "string"
          ? StellarSdk.xdr.SorobanAuthorizationEntry.fromXDR(
              authEntry,
              "base64",
            )
          : authEntry;
      if (entry.credentials().switch().name === "sorobanCredentialsAddress") {
        signedAuth.push(
          await StellarSdk.authorizeEntry(
            entry,
            keypair,
            latestLedger + 100,
            NETWORK_PASSPHRASE,
  ...
  while (txStatus === "NOT_FOUND") {
    ...
    if (txStatus === "FAILED") {
      ...
      try {
        ...
        reason = resultXdr
          ? `resultXdr: ${resultXdr.slice(0, 60)}...`
          : "no details";
      }
    ...
    if (txStatus === "SUCCESS" && pollJson?.resultMetaXdr) {
      try {
        ...
        console.warn(
          "Failed to parse actual leafIndex from meta, falling back to expected index",
          e,
        );
      }
    }
