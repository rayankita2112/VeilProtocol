...
async function buildLayers(leaves: bigint[]): Promise<bigint[][]> {
  ...
  for (let level = 0; level < SUBSET_DEPTH; level++) {
    ...
    for (let i = 0; i < currentLayer.length; i += 2) {
      ...
      const right =
        i + 1 < currentLayer.length ? currentLayer[i + 1] : zeros[level];
 ...
 */
export async function getSubsetProof(
  commitment: string,
): Promise<SubsetProofData | null> {
  const normalized = commitment
    .toLowerCase()
    .replace(/^0x/, "")
    .padStart(64, "0");
  ...
  for (let level = 0; level < SUBSET_DEPTH; level++) {
    ...
    const sibling =
      siblingIdx < layers[level].length ? layers[level][siblingIdx] : 0n;
