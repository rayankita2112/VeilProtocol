  ...
  try {
    ...
    if (body.action === "deposit") {
      return NextResponse.json(
        {
          error:
            "Deposits must be constructed client-side. The server does not generate notes.",
        },
        { status: 400 },
      );
    }
...
async function verifySubsetProof(
  ...
  try {
    ...
    if (subsetRoot !== treeInfo.root) {
      return {
        valid: false,
        error: "Subset root mismatch — tree may have been updated",
      };
    }
    ...
    try {
      ...
      const vkPath = path.join(
        process.cwd(),
        "public/circuits/subset_verification_key.json",
      );
      ...
      return {
        valid: false,
        error: "Subset verification key not found — cannot verify proof",
      };
    }
    ...
    if (!isValid) {
      return {
        valid: false,
        error: "Subset proof cryptographic verification failed",
      };
    }
...
async function handleWithdraw(body: Record<string, unknown>) {
  ...
  if (parts[0] !== "veil" || parts.length !== 5) {
    return NextResponse.json(
      {
        error:
          "Invalid note format — must be veil-<nullifier>-<secret>-<denomination>-<leafIndex>",
      },
      { status: 400 },
    );
  }
