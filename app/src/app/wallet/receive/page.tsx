...
import { formatTokenAmount, findTierByPoolId } from "@/lib/tokens";
...
function decodeClaimPayload(encoded: string): ClaimPayload | null {
  try {
    ...
    if (
      ...
      data.notes.every(
        (item: { note?: unknown; poolId?: unknown }) =>
...
function claimPayloadItems(
  payload?: ClaimPayload | null,
): { note: string; poolId: string }[] {
  if (!payload) return [];
  if (payload.notes?.length) return payload.notes;
  if (payload.note && payload.poolId)
    return [{ note: payload.note, poolId: payload.poolId }];
  return [];
}
...
function parseNoteAmount(noteString: string, poolId?: string): string {
  ...
  if (raw === null) return "?";
  const symbol = poolId
    ? (findTierByPoolId(poolId)?.tokenSymbol ?? "XLM")
    : "XLM";
...
function formatClaimItemsAmount(
  items: { note: string; poolId?: string }[],
): string {
  const totalRaw = items.reduce(
    (sum, item) => sum + (parseNoteRaw(item.note) ?? 0n),
    0n,
  );
  const symbol = items[0]?.poolId
    ? (findTierByPoolId(items[0].poolId)?.tokenSymbol ?? "XLM")
    : "XLM";
      ...
      <div className="rounded-xl border border-border/60 overflow-hidden">
        ...
        <div className="px-4 py-2.5 bg-muted/20 border-t border-border/40">
          <p className="text-[11px] text-muted-foreground text-center">
            Send from Binance, Coinbase, Yellow Card, Luno, or any Stellar
            wallet
          </p>
        </div>
      </div>
...
function ClaimTab({ initialClaim }: { initialClaim?: ClaimPayload | null }) {
  ...
  const [step, setStep] = useState<"paste" | "withdrawing" | "success">(
    "paste",
  );
  const [claimItems, setClaimItems] =
    useState<{ note: string; poolId: string }[]>(initialItems);
    ...
    try {
      ...
      for (let i = 0; i < activeClaimItems.length; i += 1) {
        ...
        const result = await executeWithdraw(
          ...
          (s) =>
            setProgress(
              activeClaimItems.length > 1
                ? `Note ${i + 1}/${activeClaimItems.length}: ${s}`
                : s,
            ),
        ...
        try {
          const tierInfo = item.poolId
            ? findTierByPoolId(item.poolId)
            : undefined;
        ...
        <>
          <div className="rounded-xl border border-border/60 p-5 space-y-3">
            ...
            <p className="text-[11px] text-muted-foreground mb-2">
              Paste a bearer note from a sender, or open a claim link they
              shared with you.
            </p>
            <textarea
              value={noteString}
            ...
            {claimItems.length <= 1 &&
              noteString &&
              noteString.startsWith("veil-") && (
                <div className="flex items-center gap-2 text-sm">
                  <Shield className="h-4 w-4 text-emerald-600" />
                  <span className="font-medium">
                    {parseNoteAmount(noteString, poolId)}
                  </span>
            ...
            {claimItems.length > 1 && (
              <div className="flex items-center gap-2 text-sm">
                <Shield className="h-4 w-4 text-emerald-600" />
                <span className="font-medium">
                  {formatClaimItemsAmount(claimItems)}
                </span>
                <span className="text-muted-foreground">
                  across {claimItems.length} notes
                </span>
              </div>
            )}
          </div>
        ...
        <div className="py-12 text-center space-y-4">
          ...
          <div>
            <h3 className="text-sm font-semibold">
              Generating ZK Proof & Withdrawing
            </h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-muted/45 px-3 py-2">
                ...
                <div className="mt-0.5 text-sm font-semibold">
                  {formatClaimItemsAmount(activeClaimItems)}
                </div>
              </div>
              <div className="rounded-lg bg-muted/45 px-3 py-2">
                <div className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
            ...
            {txHashes.length > 0 && (
              <div className="space-y-1">
                ...
                {txHashes.map((hash, index) => (
                  <div
                    key={`${hash}-${index}`}
                    className="rounded-lg bg-muted/45 px-3 py-2 font-mono text-xs truncate"
                  >
                    {hash}
                  </div>
                ))}
