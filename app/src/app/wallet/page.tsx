...
import { getActiveTokens, getPoolTiers, type PoolTier } from "@/lib/tokens";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "create") {
      ...
      try {
        ...
        if (!found) {
          setError("Vault not found — check your username");
          setLoading(false);
          return;
        }
        if (!unlockWallet(pin)) {
    ...
    <div className="py-3 border-b border-[#1e2329]/60 last:border-0">
      <div className="flex items-center gap-3">
        ...
        <div className="text-right">
          <div
            className={`text-sm font-semibold font-mono ${rowConfig.amountClass}`}
          >
            {rowConfig.prefix}
            {note.amountDisplay}
          </div>
      ...
      {showVk && vk && (
        <div className="mt-2 ml-11 rounded-lg bg-violet-500/10 border border-violet-500/20 p-2.5 flex items-center gap-2">
          ...
          <button
            onClick={() => setShowVk(false)}
            className="text-violet-500 hover:text-violet-300 shrink-0"
          >
            <X className="h-3 w-3" />
          </button>
        </div>
...
function usePublicBalance(address: string | null) {
  const [bal, setBal] = useState<PublicBalances>({
    xlm: "0",
    usdc: "0",
    usdt: "0",
    funded: false,
...
function Dashboard() {
  ...
  const hasUsdcTrustline =
    publicBal.hasUsdcTrustline || trustlineStatus === "done";
  ...
  const handleAddTrustline = async () => {
    ...
    try {
      ...
      if (!submitRes.ok) {
        ...
        throw new Error(
          err.extras?.result_codes?.operations?.[0] || "Trustline failed",
        );
      }
  ...
  const tierAmountHuman = selectedTier
    ? parseFloat(selectedTier.amount) / 1e7
    : 0;
  const hasEnough = selectedTier
    ? publicBalForToken(selectedToken) >=
      tierAmountHuman + (selectedToken === "XLM" ? 1 : 0)
    : false;
  ...
  const publicBalSummary = () => {
    ...
    if (xlmNum > 0)
      parts.push(
        `${xlmNum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} XLM`,
      );
    if (usdcNum > 0)
      parts.push(
        `${usdcNum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC`,
      );
    if (usdtNum > 0)
      parts.push(
      ...
      {publicBal.funded && (
        <div className="rounded-xl border border-[#1e2329] bg-[#131722] overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#1e2329] flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono text-[#848e9c]/60 uppercase">
              Select token & amount
            </span>
          </div>
          ...
          <div className="p-5 space-y-4">
              ...
              publicBal.funded && (
                <div className="rounded-lg bg-[#181c25] border border-[#1e2329] p-4 space-y-3">
                  ...
                  {!publicBal.hasUsdcTrustline && selectedToken === "USDC" ? (
                    ...
                    >
                      {trustlineStatus === "adding" ? (
                        ...
                        <>
                          <Check className="h-3.5 w-3.5" /> Trustline added! Now
                          get USDC below
                        </>
                  ...
                  <p className="text-[9px] font-mono text-[#848e9c]/50 text-center">
                    Select &quot;Stellar&quot; on the faucet, paste your
                    address, get 20 {selectedToken}
                  </p>
                </div>
              )}
            ...
            <button
              ...
              disabled={
                !selectedTier || depositStatus === "depositing" || !hasEnough
              }
            ...
            >
              ...
              {depositStatus === "depositing" &&
                `Shielding ${selectedTier?.label}...`}
              {depositStatus === "done" &&
                `Shielded ${selectedTier?.label} \u2713`}
            ...
            {depositStatus === "error" && depositError && (
              <p className="text-[10px] font-mono text-red-400/70 text-center truncate">
                {depositError}
              </p>
            )}
          </div>
      ...
      <div className="rounded-xl border border-[#1e2329] bg-[#131722] shadow-sm">
        ...
        <div className="px-5">
          {activity.length === 0 ? (
            ...
            activity
              .slice(0, 5)
              .map((note) => <ActivityRow key={note.id} note={note} />)
          )}
        </div>
        {unspent.length > 0 && (
          <Link
