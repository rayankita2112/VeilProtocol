...
import { Lock, Unlock, Clock, Key, CheckCircle2 } from "lucide-react";
      ...
      <div className="max-w-2xl mx-auto px-4 lg:px-6 py-8 pb-24 lg:pb-12 space-y-6">
        ...
        <div className="pb-5 border-b border-[#1e2329]">
          ...
          <p className="text-sm font-mono text-[#848e9c] leading-relaxed max-w-xl">
            Authorized auditors can paste a reveal key to inspect transaction
            metadata once the mandatory timelock delay has elapsed.
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            {loaded ? (
              <Button
                variant="outline"
                onClick={handleReset}
                className="h-12 px-6 border-[#1e2329] text-[#eaecef] hover:bg-[#181c25] font-mono"
              >
                Clear Key
              </Button>
            ) : (
              <Button
                ...
                className="h-12 px-6 bg-[#f7a600] text-[#0b0e11] font-mono font-bold hover:bg-[#f7a600]/90"
              >
                Verify & Decrypt
              </Button>
            )}
        ...
        {loaded && records.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-[#1e2329] pb-3">
              <h3 className="text-sm font-mono font-bold text-[#eaecef] uppercase tracking-wider">
                Decrypted Record
              </h3>
            ...
            <div className="space-y-4">
              {records.map((rec) => (
                <div
                  key={rec.id}
                  className="rounded-xl border border-[#1e2329] bg-[#131722] p-6 shadow-sm"
                >
                  {rec.status === "timelocked" ? (
                    <div>
                      ...
                      <div className="p-4 rounded-lg bg-[#181c25] border border-[#f7a600]/40 text-center mb-2">
                        <div className="text-2xl font-mono font-bold text-[#f7a600] tracking-tight">
                          04 : 00 : 00
                        </div>
                        <div className="text-xs font-mono text-[#848e9c] mt-1">
                          Remaining until decryption authorized
                        </div>
                      </div>
                      <p className="text-xs font-mono text-[#848e9c]">
                        Note Hash:{" "}
                        <code className="text-[#eaecef]">{rec.noteHash}</code>
                      </p>
                    </div>
                  ) : (
                    <div>
        ...
        <div className="rounded-xl border border-[#1e2329] bg-[#131722] p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs font-mono text-[#848e9c] leading-relaxed">
            <div>
              ...
              Reveal keys decrypt note metadata but have zero spending
              authority.
            </div>
            <div>
              <strong className="text-[#eaecef] block mb-1">Timelocked</strong>
              Mandatory delay (24h–72h) before decryption is authorized
              on-chain.
            </div>
            <div>
